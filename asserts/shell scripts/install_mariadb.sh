sudo cat > /etc/yum.repos.d/mariadb.repo << EFF
[mariadb]
name = MariaDB
...
sudo cp /etc/my.cnf.d/server.cnf{,.orig} 

sudo cat > /etc/my.cnf.d/server.cnf <<EFF
...
slow-query-log = 1
#slow-query-log-file = /data/mariadb/logs/mysql-slow.log
long_query_time = 5
